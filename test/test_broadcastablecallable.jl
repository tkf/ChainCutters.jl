...
@testset "WeightedAdd" begin
    ...
    y_actual, back_actual = Zygote.pullback(f -> sum(f.(cut(u), cut(v))), f)
    y_desired, back_desired = Zygote.pullback(f -> sum(f.(u, v)), f)
    @test y_actual == y_desired
    @test back_actual(1) isa Tuple{NamedTuple{(:a, :b)}}
    @test back_actual(1) == back_desired(1)

    y_partialcut, back_partialcut = Zygote.pullback(f) do f
        @set! f.b = cut(f.b)
        sum(f.(cut(u), cut(v)))
    end
...
@testset "ArgumentError" begin
    ...
    err = nothing
    try
        Zygote.pullback(x -> f.(x), Ref([0.0]))
    catch err
    end
    @test err isa ArgumentError
    @test occursin("Use `cut`", sprint(showerror, err))

    y, back = Zygote.pullback(x -> cut(f).(cut(x)), Ref([0.0]))
    @test y == 1
    @test back(1) === (nothing,)
end
...
@testset "NonDifferentiableType" begin
    f = AddCall(Poly3(rand(4)...), Poly3(rand(4)...))
    y, back = Zygote.pullback(x -> cut(f).(x), missing)
    @test y === missing
    @test back(1) === (nothing,)
end
